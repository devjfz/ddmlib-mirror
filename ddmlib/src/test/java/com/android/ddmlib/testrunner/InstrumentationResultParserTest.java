...
public class InstrumentationResultParserTest extends TestCase {
   ...
   *
   * <p>This mimics launch performance test output.
   */
  ...
  public void testParse_invalidCode() {
    ...
    addLine(
        output, "INSTRUMENTATION_STATUS: Error=Unable to find instrumentation target package: foo");
    addLine(
        output,
  ...
  /**
   * Test parsing output for a test run failure, where an instrumentation component failed to load.
   *
   * <p>Parsing input takes the from of INSTRUMENTATION_RESULT: fff
   */
   ...
   *
   * <p>This can occur if device spontaneously reboots, or if test method could not be found.
   */
  public void testParse_incomplete() {
    ...
    mMockListener.testFailed(
        EasyMock.eq(TestFailure.ERROR),
        EasyMock.eq(TEST_ID),
        EasyMock.startsWith(InstrumentationResultParser.INCOMPLETE_TEST_ERR_MSG_PREFIX));
    mMockListener.testEnded(TEST_ID, Collections.EMPTY_MAP);
    mMockListener.testRunFailed(
        EasyMock.startsWith(InstrumentationResultParser.INCOMPLETE_RUN_ERR_MSG_PREFIX));
    mMockListener.testRunEnded(0, Collections.EMPTY_MAP);
  ...
  private void addStatusKey(StringBuilder outputBuilder, String key, String value) {
  ...
  private void addResultKey(StringBuilder outputBuilder, String key, String value) {
