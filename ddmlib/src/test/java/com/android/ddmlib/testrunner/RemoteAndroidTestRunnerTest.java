...
public class RemoteAndroidTestRunnerTest extends TestCase {
  ...
  public void testRun() throws Exception {
    String expectedCmd =
        EasyMock.eq(String.format("am instrument -w -r  %s/%s", TEST_PACKAGE, TEST_RUNNER));
  ...
  public void testRun_withMethod() throws Exception {
    ...
    String expectedCmd = EasyMock.contains(String.format("-e class %s#%s", className, testName));
    runAndVerify(expectedCmd);
  }
  ...
  public void testRun_withAddInstrumentationArg() throws Exception {
    ...
    String expectedCmd = EasyMock.contains(String.format("-e %s %s", extraArgName, extraArgValue));
    runAndVerify(expectedCmd);
  }
  ...
  public void testRun_ioException() throws Exception {
    mMockDevice.executeShellCommand(
        (String) EasyMock.anyObject(), (IShellOutputReceiver) EasyMock.anyObject(), EasyMock.eq(0));
  ...
  private void runAndVerify(String expectedCmd) throws Exception {
    mMockDevice.executeShellCommand(
        expectedCmd, (IShellOutputReceiver) EasyMock.anyObject(), EasyMock.eq(0));
