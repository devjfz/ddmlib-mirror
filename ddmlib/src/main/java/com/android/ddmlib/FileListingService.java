...
public final class FileListingService {
  ...
  private static final Pattern LS_L_PATTERN =
      Pattern.compile(
          "^([bcdlsp-][-r][-w][-xsS][-r][-w][-xsS][-r][-w][-xstST])\\s+(\\S+)\\s+(\\S+)\\s+"
              + "([\\d\\s,]*)\\s+(\\d{4}-\\d\\d-\\d\\d)\\s+(\\d\\d:\\d\\d)\\s+(.*)$"); //$NON-NLS-1$

  private static final Pattern LS_LD_PATTERN =
      Pattern.compile("d[rwx-]{9}\\s+\\S+\\s+\\S+\\s+[0-9-]{10}\\s+\\d{2}:\\d{2}$"); // $NON-NLS-1$
  ...
  public static final class FileEntry {
     ...
     *
     * <p>For an application apk file it will be the application package as returned by the Package
     * Manager.
     */
    ...
    /**
     * Returns the cached children of the entry. This returns the cache created from calling <code>
     * FileListingService.getChildren()</code>.
     */
    ...
    /**
     * Returns the child {@link FileEntry} matching the name. This uses the cached children list.
     *
  ...
  private static class LsReceiver extends MultiLineReceiver {
     ...
     * @param linkList the list of link path to compute post ls, to figure out if the link pointed
   ...
   *
   * @see FileListingService#getChildren(com.android.ddmlib.FileListingService.FileEntry, boolean,
   ...
   *
   * @return the {@link FileEntry} object representing the root element or <code>null</code> if the
   *     device is invalid.
   */
  public FileEntry getRoot() {
    if (mDevice != null) {
      if (mRoot == null) {
        mRoot = new FileEntry(null /* parent */, "" /* name */, TYPE_DIRECTORY, true /* isRoot */);
      }
   ...
   *
   * <p>If <var>receiver</var> is <code>null</code>, the device side <code>ls</code> command is done
   * synchronously, and the method will return upon completion of the command.<br>
   ...
   * <var>useCache</var> allows usage of this cache, but only if the cache is valid. The cache is
   * valid only for {@link FileListingService#REFRESH_RATE} ms. After that a new <code>ls</code>
