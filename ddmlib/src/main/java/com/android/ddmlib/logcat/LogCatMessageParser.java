...
public final class LogCatMessageParser {
  ...
  /**
   * This pattern is meant to parse the first line of a log message with the option 'logcat -v
   * long'. The first line represents the date, tag, severity, etc.. while the following lines are
   * the message (can be several lines).<br>
   * This first line looks something like:<br>
   * {@code "[ 00-00 00:00:00.000 <pid>:0x<???> <severity>/<tag>]"} <br>
   * Note: severity is one of V, D, I, W, E, A? or F. However, there doesn't seem to be a way to
   * actually generate an A (assert) message. Log.wtf is supposed to generate a message with
   * severity A, however it generates the undocumented F level. In such a case, the parser will
   * change the level from F to A.<br>
  ...
  public List<LogCatMessage> processLogLines(String[] lines, IDevice device) {
    ...
    for (String line : lines) {
      ...
      if (matcher.matches()) {
        ...
        LogCatMessage m =
            new LogCatMessage(mCurLogLevel, mCurPid, mCurTid, pkgName, mCurTag, mCurTime, line);
        messages.add(m);
      }
    }
