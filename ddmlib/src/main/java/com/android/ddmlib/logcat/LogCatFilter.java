...
/**
 * A Filter for logcat messages. A filter can be constructed to match different fields of a logcat
 * message. It can then be queried to see if a message matches the filter's settings.
 */
public final class LogCatFilter {
  ...
  /**
   * Construct a filter with the provided restrictions for the logcat message. All the text fields
   * accept Java regexes as input, but ignore invalid regexes.
   *
   ...
   * @param logLevel value for the logcat message's log level. Only messages of higher priority will
  ...
  /**
   * Obtain the flags to pass to {@link Pattern#compile(String, int)}. This method tries to figure
   * out whether case sensitive matching should be used. It is based on the following heuristic: if
   * the regex has an upper case character, then the match will be case sensitive. Otherwise it will
   * be case insensitive.
   */
   ...
   *
   * @param query encoded search string. The query is simply a list of words (can be regexes) a user
   *     would type in a search bar. These words are searched for in the text field of each
   *     collected logcat message. To search in a different field, the word could be prefixed with a
   *     keyword corresponding to the field name. Currently, the following keywords are supported:
   *     "pid:", "tag:" and "text:". Invalid regexes are ignored.
