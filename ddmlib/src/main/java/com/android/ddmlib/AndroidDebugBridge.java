...
public final class AndroidDebugBridge {
  ...
  /**
   * Classes which implement this interface provide methods that deal with {@link IDevice} addition,
  ...
  public interface IClientChangeListener {
     ...
     *     following values: {@link Client#CHANGE_INFO}, {@link Client#CHANGE_DEBUGGER_STATUS},
     *     {@link Client#CHANGE_THREAD_MODE}, {@link Client#CHANGE_THREAD_DATA}, {@link
   ...
   *
   * @param clientSupport Indicates whether the library should enable the monitoring and interaction
   *     with applications running on the devices.
   * @see #init(boolean)
   */
   ...
   *       The library monitors the devices and the applications running on them. It will connect to
   *       each application, as a debugger of sort, to be able to interact with them through JDWP
   *       packets.
   ...
   *
   * <p>Note that mode 1 does not prevent debugging of applications running on devices. Mode 1 lets
   * debuggers connect to <code>ddmlib</code> which acts as a proxy between the debuggers and the
   * applications to debug. See {@link Client#getDebuggerListenPort()}.
   *
  ...
  /**
   * Returns whether the ddmlib is setup to support monitoring and interacting with {@link Client}s
   * running on the {@link IDevice}s.
   */
  ...
  public static AndroidDebugBridge createBridge() {
    synchronized (sLock) {
      ...
      IDebugBridgeChangeListener[] listenersCopy =
          sBridgeListeners.toArray(new IDebugBridgeChangeListener[sBridgeListeners.size()]);
   ...
   * @param forceNewBridge force creation of a new bridge even if one with the same location already
   *     exists.
   * @return a connected bridge.
   */
   ...
   *
   * <p>A new object will have to be created with {@link #createBridge(String, boolean)}.
   */
  ...
  /**
   * Adds the listener to the collection of listeners who will be notified when a {@link IDevice} is
   * connected, disconnected, or when its properties or its {@link Client} list changed, by sending
   * it one of the messages defined in the {@link IDeviceChangeListener} interface.
   *
   * @param listener The listener which should be notified.
   */
   ...
   * IDevice} is connected, disconnected, or when its properties or its {@link Client} list changed.
   *
   * @param listener The listener which should no longer be notified.
   */
  ...
  /**
   * Returns the number of times the {@link AndroidDebugBridge} object attempted to restart the adb
   * daemon.
   */
  ...
  /**
   * Queries adb for its version number and checks it against {@link #MIN_VERSION_NUMBER} and {@link
   * #MAX_VERSION_NUMBER}
   */
  private void checkAdbVersion() {
    ...
    int status;
    try {
      status = grabProcessOutput(process, errorOutput, stdOutput, true /* waitForReaders */);
    ...
    if (!versionFound) {
      ...
      StringBuilder builder =
          new StringBuilder("Failed to parse the output of 'adb version':\n"); // $NON-NLS-1$
   ...
   *
   * <p>If a version number is found, it checks the version number against what is expected by this
   * version of ddms.
   *
   * <p>Returns true when a version number has been found so that we can stop scanning, whether the
   * version number is in the acceptable range or not.
   *
   ...
   *
   * <p>The notification of the listeners is done in a synchronized block. It is important to expect
   * the listeners to potentially access various methods of {@link IDevice} as well as {@link
   * #getDevices()} which use internal locks.
   *
   ...
   *
   * <p>This includes adding/removing listeners, but also notifying listeners of new bridges,
   * devices, and clients.
   */
   ...
   *
   * <p>The value for the environment variable ANDROID_ADB_SERVER_PORT is validated,
