...
class Debugger {
   ...
   * admitting to have done so. We also send the handshake response to the debugger, along with any
   * pending pre-connection data, which is why this can throw an IOException.
   */
   ...
   *
   * <p>"mClient" will never be null, though it's possible that the channel in the client has closed
   * and our send attempt will fail.
   *
  ...
  /**
   * Send the handshake to the debugger. We also send along any packets we already received from the
   * client (usually just a VM_START event, if anything at all).
   */
