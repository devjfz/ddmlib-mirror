 ...
 *
 * <p>This {@link DebugPortManager} allows applications using ddmlib to provide a custom port
 * provider on a per-<code>Client</code> basis, depending on the device/emulator they are running
 * on, and/or their names.
 */
public class DebugPortManager {

  /**
   * Classes which implement this interface provide a method that provides a non random debugger
   * port for a newly created {@link Client}.
   */
  public interface IDebugPortProvider {
    ...
    /**
     * Returns a non-random debugger port for the specified application running on the specified
     * {@link Device}.
     *
     ...
     * @return The non-random debugger port or {@link #NO_STATIC_PORT} if the {@link Client} should
     *     use the automatic debugger port provider.
     */
  ...
  /**
   * Sets the {@link IDebugPortProvider} that will be used when a new {@link Client} requests a
   * debugger port.
   *
