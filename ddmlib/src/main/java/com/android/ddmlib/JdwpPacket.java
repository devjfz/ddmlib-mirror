...
final class JdwpPacket {
   ...
   * "slice" returned by getPayload(). We could fish it out of the chunk header, but it's legal for
   * there to be more than one chunk in a JDWP packet.
   *
   ...
   * just for the current connection. This is a useful property when debugging, but isn't necessary.
   *
   ...
   * header. The buffer's limit will be set to the size of the payload if the size is known; if this
   * is a packet under construction the limit will be set to the end of the buffer.
   *
  ...
  /**
   * Return the packet's ID. For a reply packet, this allows us to match the reply with the original
   * request.
   */
  int getId() {
    return mId;
  ...
  /**
   * Write our packet to "chan". Consumes the packet as part of the write.
   *
  ...
  /**
   * Find the JDWP packet at the start of "buf". The start is known, but the length has to be parsed
   * out.
   *
   ...
   *
   * <p>Returns a new JdwpPacket if a full one is found in the buffer. If not, returns null. Throws
   * an exception if the data doesn't look like a valid JDWP packet.
   */
  static JdwpPacket findPacket(ByteBuffer buf) {
    int count = buf.position();
    int length, id, flags, cmdSet, cmd;

    if (count < JDWP_HEADER_LEN) return null;
  ...
  static int findHandshake(ByteBuffer buf) {
    ...
    if (count < mHandshake.length) return HANDSHAKE_NOTYET;

    for (i = mHandshake.length - 1; i >= 0; --i) {
      if (buf.get(i) != mHandshake[i]) return HANDSHAKE_BAD;
    }
