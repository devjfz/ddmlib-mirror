...
/**
 * A Device monitor. This connects to the Android Debug Bridge and get device and debuggable process
 * information from it.
 */
final class DeviceMonitor {
    ...
    if (length > 0) {
      ...
      for (String d : devices) {
        ...
        if (param.length == 2) {
          ...
          Device device =
              new Device(this, param[0] /*serialnumber*/, DeviceState.getState(param[1]));
  ...
  private void queryNewDeviceForInfo(Device device) {
    ...
    try {
      ...
      Log.w(
          "DeviceMonitor",
          String.format("Connection timeout getting info for device %s", device.getSerialNumber()));
      ...
      Log.w(
          "DeviceMonitor",
          String.format("IO Error getting info for device %s", device.getSerialNumber()));
    }
  }
  ...
  private boolean startMonitoringDevice(Device device) {
    ...
    if (socketChannel != null) {
      try {
        ...
        Log.d(
            "DeviceMonitor",
            "Connection Failure when starting to monitor device '" + device + "' : timeout");
      } catch (AdbCommandRejectedException e) {
        try {
          // attempt to close the socket if needed.
          socketChannel.close();
  ...
  private void openClient(Device device, int pid, int port, MonitorThread monitorThread) {
    ...
    try {
      clientSocket =
          AdbHelper.createPassThroughConnection(AndroidDebugBridge.getSocketAddress(), device, pid);
  ...
  private void createClient(
      Device device, int pid, SocketChannel socket, int debuggerPort, MonitorThread monitorThread) {
