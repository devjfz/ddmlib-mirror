 ...
 *
 * <p>This is a stand-alone object, not linked to a particular Event. It describes the value, by
 * name, type ({@link EventValueType}), and (if needed) value unit ({@link ValueType}).
 *
 * <p>The index of the value is not contained within this class, and is instead dependent on the
 * index of this particular object in the array of {@link EventValueDescription} returned by {@link
...
public final class EventValueDescription {
  ...
  public static enum ValueType {
    ...
    public void checkType(EventValueType type) throws InvalidValueTypeException {
      if ((type != EventValueType.INT && type != EventValueType.LONG) && this != NOT_APPLICABLE) {
    ...
    /**
     * Returns a {@link ValueType} from an integer value, or <code>null</code> if no match were
     * found.
     *
   ...
   *
   * <p>If the type is {@link EventValueType#INT} or {@link EventValueType#LONG}, the {@link
   * #mValueType} is set to {@link ValueType#BYTES} by default. It set to {@link
   * ValueType#NOT_APPLICABLE} for all other {@link EventValueType} values.
   *
   * @param name
   * @param type
   */
  ...
  public String toString() {
    if (mValueType != ValueType.NOT_APPLICABLE) {
      return String.format(
          "%1$s (%2$s, %3$s)", mName, mEventValueType.toString(), mValueType.toString());
    }
  ...
  /**
   * Returns an object of a valid type (based on the value returned by {@link #getEventValueType()})
   * from a String value.
   *
   * <p>IMPORTANT {@link EventValueType#LIST} and {@link EventValueType#TREE} are not supported.
   *
