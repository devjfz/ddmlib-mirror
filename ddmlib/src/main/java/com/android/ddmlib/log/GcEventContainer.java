...
/**
 * Custom Event Container for the Gc event since this event doesn't simply output data in int or
 * long format, but encodes several values on 4 longs.
 *
 * <p>The array of {@link EventValueDescription}s parsed from the "event-log-tags" file must be
 * ignored, and instead, the array returned from {@link #getValueDescriptions()} must be used.
 */
final class GcEventContainer extends EventContainer {
  ...
  /**
   * Returns a custom array of {@link EventValueDescription} since the actual content of this event
   * (list of (long, long) does not match the values encoded into those longs.
   */
  static EventValueDescription[] getValueDescriptions() {
    try {
      return new EventValueDescription[] {
        ...
        new EventValueDescription(
            "Allocated Objects (aggregate)", EventValueType.LONG, ValueType.OBJECTS),
        new EventValueDescription(
            "Allocated Bytes (aggregate)", EventValueType.LONG, ValueType.BYTES),
        ...
        new EventValueDescription(
            "Allocated Objects (zygote)", EventValueType.LONG, ValueType.OBJECTS),
        new EventValueDescription("Allocated Bytes (zygote)", EventValueType.LONG, ValueType.BYTES),
        new EventValueDescription(
            "External Allocation Limit", EventValueType.LONG, ValueType.BYTES),
        ...
        new EventValueDescription(
            "Malloc Info: Total Allocated Space", EventValueType.LONG, ValueType.BYTES),
      };
   ...
   * @param offset the offset in the array where to put the value. Array length must be at least
   *     offset + 8
   */
