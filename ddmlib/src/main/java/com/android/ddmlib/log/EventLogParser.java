...
public final class EventLogParser {
  ...
  private static final Pattern PATTERN_SIMPLE_TAG =
      Pattern.compile("^(\\d+)\\s+([A-Za-z0-9_]+)\\s*$"); // $NON-NLS-1$
  private static final Pattern PATTERN_TAG_WITH_DESC =
      Pattern.compile("^(\\d+)\\s+([A-Za-z0-9_]+)\\s*(.*)\\s*$"); // $NON-NLS-1$
  private static final Pattern PATTERN_DESCRIPTION =
      Pattern.compile("\\(([A-Za-z0-9_\\s]+)\\|(\\d+)(\\|\\d+){0,1}\\)"); // $NON-NLS-1$

  private static final Pattern TEXT_LOG_LINE =
      Pattern.compile(
   ...
   *
   * <p>This methods reads the event-log-tags located on the device to find out what tags are being
   * written to the event log and what their format is.
   *
      ...
      if (m.matches()) {
        try {
          ...
          if (value == GcEventContainer.GC_EVENT_TAG) {
            mValueDescriptionMap.put(value, GcEventContainer.getValueDescriptions());
          } else {
   ...
   *
   * <p>This needs to be recursive because you can have lists of lists.
   *
   * <p>If we run out of room, we stop processing immediately. It's important for us to check for
   * space on every output element to avoid producing garbled output.
   *
   * <p>Returns the amount read on success, -1 on failure.
   */
  private static int parseBinaryEvent(byte[] eventData, int dataOffset, ArrayList<Object> list) {

    if (eventData.length - dataOffset < 1) return -1;
      ...
      case EVENT_TYPE_INT:
        {
          ...
          if (eventData.length - offset < 4) return -1;
      ...
      case EVENT_TYPE_LONG:
        {
          ...
          if (eventData.length - offset < 8) return -1;
      ...
      case EVENT_TYPE_STRING:
        {
          ...
          if (eventData.length - offset < strLen) return -1;
      ...
      case EVENT_TYPE_LIST:
        {
          ...
          if (eventData.length - offset < 1) return -1;
