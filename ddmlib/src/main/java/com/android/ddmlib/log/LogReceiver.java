...
public final class LogReceiver {
  ...
  /**
   * Classes which implement this interface provide a method that deals with {@link LogEntry}
   * objects coming from log service through a {@link LogReceiver}.
   *
   * <p>This interface provides two methods.
   *
   * <ul>
   *   <li>{@link #newEntry(com.android.ddmlib.log.LogReceiver.LogEntry)} provides a first level of
   *       parsing, extracting {@link LogEntry} objects out of the log service output.
   *   <li>{@link #newData(byte[], int, int)} provides a way to receive the raw information coming
   *       directly from the log service.
   * </ul>
   */
   ...
   *
   * <p>The {@link ILogListener} will receive new log entries as they are parsed, in the form of
  ...
  /**
   * Creates a {@link LogEntry} from the array of bytes. This expects the data buffer size to be at
   * least <code>offset + {@link #ENTRY_HEADER_SIZE}</code>.
   *
