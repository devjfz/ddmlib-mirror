...
public final class EmulatorConsole {
  ...
  private static final Pattern sDataStatusRegexp =
      Pattern.compile("gsm\\s+data\\s+state:\\s*([a-z]+)", Pattern.CASE_INSENSITIVE); // $NON-NLS-1$
  ...
  /**
   * Returns an {@link EmulatorConsole} object for the given {@link Device}. This can be an already
   * existing console, or a new one if it hadn't been created yet.
   *
   ...
   *
   * @return a {@link NetworkStatus} object containing the {@link GsmStatus}, or <code>null</code>
   *     if the query failed.
   */
   ...
   *
   * @return a {@link GsmStatus} object containing the gms status, or <code>null</code> if the query
   *     failed.
   */
   ...
   * @param message The SMS message. \ characters must be escaped. The carriage return is the 2
