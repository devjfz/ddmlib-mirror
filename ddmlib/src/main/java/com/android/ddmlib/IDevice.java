...
public interface IDevice extends IShellEnabledDevice {
  ...
  @Deprecated public static final String PROP_BUILD_VERSION_NUMBER = PROP_BUILD_API_LEVEL;
  ...
  /**
   * A variant of {@link #getProperty(String)} that will attempt to retrieve the given property from
   * device directly, without using cache.
   *
   * @param name the name of the value to return.
   ...
   * attempt to retrieve the property from cache if available, and if not, will query the device
   * directly.
   *
   * @param name the name of the value to return.
   * @return the value or <code>null</code> if the property does not exist
   ...
   *
   * @return <code>null</code> if the SyncService couldn't be created. This can happen if adb refuse
   *     to open the connection because the {@link IDevice} is invalid (or got disconnected).
   * @throws TimeoutException in case of timeout on the connection.
   * @throws AdbCommandRejectedException if adb rejects the command
   ...
   *
   * <p>The battery level may be cached. Only queries the device for its battery level if 5 minutes
   * have expired since the last successful query.
   *
