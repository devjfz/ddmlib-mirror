 ...
 *
 * <p>{@link DebugBridgeServer} is the public API to connection to adb, while {@link AdbHelper} does
 * the low level stuff.
 *
...
final class AdbHelper {
   ...
   *
   * @param adbSockAddr
   * @param device the device to connect to. Can be null in which case the connection will be to the
  ...
  public static SocketChannel createPassThroughConnection(
      InetSocketAddress adbSockAddr, Device device, int pid)
   ...
   * rest of the string, encoded as ASCII hex (case doesn't matter). "port" and "host" are what we
   * want to forward to. If we're on the host side connecting into the device, "addrStr" should be
   * null.
   */
   ...
   * @param readDiagString If true, we're expecting an OKAY response to be followed by a diagnostic
   *     string. Otherwise, we only expect the diagnostic string to follow a FAIL.
   * @throws TimeoutException in case of timeout on the connection.
   * @throws IOException in case of I/O error on the connection.
   */
   ...
   *     command output, the method will throw {@link ShellCommandUnresponsiveException}. A value of
   *     0 means the method will wait forever for command output and never throw.
   * @throws TimeoutException in case of timeout on the connection when sending the command.
   * @throws AdbCommandRejectedException if adb rejects the command
   * @throws ShellCommandUnresponsiveException in case the shell command doesn't send any output for
   *     a period longer than <var>maxTimeToOutputResponse</var>.
   * @throws IOException in case of I/O error on the connection.
  ...
  /**
   * Runs the Event log service on the {@link Device}, and provides its output to the {@link
   * LogReceiver}.
   *
  ...
  public static void runEventLogService(
      InetSocketAddress adbSockAddr, Device device, LogReceiver rcvr)
      throws TimeoutException, AdbCommandRejectedException, IOException {
  ...
  public static void runLogService(
      InetSocketAddress adbSockAddr, Device device, String logName, LogReceiver rcvr)
      throws TimeoutException, AdbCommandRejectedException, IOException {
    SocketChannel adbChan = null;
   ...
   * @param localPortSpec specification of the local port to forward, should be of format tcp:<port
   *     number>
  ...
  public static void createForward(
      InetSocketAddress adbSockAddr, Device device, String localPortSpec, String remotePortSpec)
      throws TimeoutException, AdbCommandRejectedException, IOException {
  ...
  /**
   * Reads from the socket until the array is filled, or no more data is coming (because the socket
   * closed or the timeout expired).
   *
   ...
   * data is coming (because the socket closed or the timeout expired). After "timeout" milliseconds
   * since the previous successful read, this will return whether or not new data has been found.
   *
   * @param chan the opened socket to read from. It must be in non-blocking mode for timeouts to
   *     work
   * @param data the buffer to store the read data into.
  ...
  /**
   * Write until all data in "data" is written, the optional length is reached, the timeout expires,
   * or the connection fails. Returns "true" if all data was written.
   *
   * @param chan the opened socket to write to.
   * @param data the buffer to send.
