 ...
 *
 * <p>Default behavior outputs the log to {@link System#out}. Use {@link
 * #setLogOutput(com.android.ddmlib.Log.ILogOutput)} to redirect the log somewhere else.
 */
public final class Log {
  ...
  public enum LogLevel {
     ...
     * Returns the {@link LogLevel} enum matching the specified letter.
     *
     * <p>The letter is passed as a {@link String} argument, but only the first character is used.
     *
     * @param letter the letter matching a <code>LogLevel</code> enum
     * @return a <code>LogLevel</code> object or <code>null</code> if no match were found.
     */
   ...
   * Show hex dump.
   *
   * <p>Local addition. Output looks like: 1230- 00 11 22 33 44 55 66 77 88 99 aa bb cc dd ee ff
   * 0123456789abcdef
   *
  ...
  static void hexDump(String tag, LogLevel level, byte[] data, int offset, int length) {
    ...
    while (length != 0) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (val >= 0x20 && val < 0x7f) line[kAscOffset + i] = (char) val;
        else line[kAscOffset + i] = '.';
      }
