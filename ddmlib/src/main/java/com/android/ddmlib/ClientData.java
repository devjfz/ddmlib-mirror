...
public class ClientData {
  ...
  public static enum DebuggerStatus {
    ...
    DEFAULT,
    /**
     * Debugger connection status: the application's VM is paused, waiting for a debugger to connect
    ...
    ATTACHED,
    /**
     * Debugger connection status: The listening port for debugger connection failed to listen. No
     * debugger will be able to connect.
     */
    ERROR
  }
  ...
  public static enum AllocationTrackingStatus {
     ...
     *
     * <p>This happens right after a {@link Client} is discovered by the {@link AndroidDebugBridge},
     * and before the {@link Client} answered the query regarding its allocation tracking status.
     *
  ...
  public static enum MethodProfilingStatus {
     ...
     *
     * <p>This happens right after a {@link Client} is discovered by the {@link AndroidDebugBridge},
     * and before the {@link Client} answered the query regarding its method profiling status.
     *
  ...
  /**
   * Name of the value representing the max size of the heap, in the {@link Map} returned by {@link
   * #getVmHeapInfo(int)}
   */
  public static final String HEAP_MAX_SIZE_BYTES = "maxSizeInBytes"; // $NON-NLS-1$
  /**
   * Name of the value representing the size of the heap, in the {@link Map} returned by {@link
   * #getVmHeapInfo(int)}
   */
  public static final String HEAP_SIZE_BYTES = "sizeInBytes"; // $NON-NLS-1$
  /**
   * Name of the value representing the number of allocated bytes of the heap, in the {@link Map}
   * returned by {@link #getVmHeapInfo(int)}
   */
  public static final String HEAP_BYTES_ALLOCATED = "bytesAllocated"; // $NON-NLS-1$
  /**
   * Name of the value representing the number of objects in the heap, in the {@link Map} returned
   * by {@link #getVmHeapInfo(int)}
   */
   ...
   */
  private ArrayList<NativeLibraryMapInfo> mNativeLibMapInfo = new ArrayList<NativeLibraryMapInfo>();
   ...
   *
   * @return the client description or <code>null</code> if not the description was not yet sent by
   *     the client.
   */
   ...
   * device, we just don't allow an empty name to replace a specified one.
   */
   ...
   * @param objectsAllocated The number of objects currently allocated in the heap
   */
  // TODO: keep track of timestamp, reason
  synchronized void setHeapInfo(
      int heapId,
      long maxSizeInBytes,
      long sizeInBytes,
      long bytesAllocated,
