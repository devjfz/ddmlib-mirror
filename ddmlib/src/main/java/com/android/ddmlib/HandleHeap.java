...
final class HandleHeap extends ChunkHandler {
   ...
   * failure response.
   *
   * <p>This is more expensive for the device than HPDU, because the entire heap dump is held in RAM
   * instead of spooled out to a temp file. On the other hand, permission to write to /sdcard is not
   * required.
   *
   * @param fileName name of output file (on device)
   */
   ...
   *
   * <p>This is just a serial collection of strings, each of which is a four-byte length followed by
   * UTF-16 data.
   */
  ...
  private void handleREAL(Client client, ByteBuffer data) {
    ...
    for (int i = 0; i < numEntries; i++) {
      ...
      list.add(
          new AllocationInfo(
              allocNumber--, classNames[classNameIndex], totalSize, (short) threadId, steArray));
    }
    ...
    for (AllocationInfo rec : records) {
      ...
      for (StackTraceElement ste : rec.getStackTrace()) {
        if (ste.isNativeMethod()) {
          System.out.println(
              "    " + ste.getClassName() + "." + ste.getMethodName() + " (Native method)");
        } else {
          System.out.println(
              "    "
                  + ste.getClassName()
