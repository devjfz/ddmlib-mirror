 ...
 *
 * <p>To get a {@link SyncService} object, use {@link Device#getSyncService()}.
 */
public final class SyncService {
  ...
  */
   ...
   * @param localPath The local destination. If the entries count is > 1 or if the unique entry is a
   *     folder, this should be a folder.
  ...
  private void doPushFile(String localPath, String remotePath, ISyncProgressMonitor monitor)
    ...
    if (!checkResult(result, ID_OKAY)) {
      throw new SyncException(SyncError.TRANSFER_PROTOCOL_ERROR, readErrorMessage(result, timeOut));
    }
  }
   ...
   * @return an Integer containing the mode if all went well or null otherwise
   * @throws IOException
   * @throws TimeoutException in case of a timeout reading responses from the device.
   */
