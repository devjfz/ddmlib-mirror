...
public class RemoteAndroidTestRunner implements IRemoteAndroidTestRunner {
   ...
   * @param runnerName the instrumentation test runner to execute. If null, will use default runner
  ...
  public void run(Collection<ITestRunListener> listeners)
      throws TimeoutException, AdbCommandRejectedException, ShellCommandUnresponsiveException,
          IOException {
    final String runCaseCommandStr =
        String.format("am instrument -w -r %1$s %2$s", getArgsCommand(), getRunnerPath());
    Log.i(
        LOG_TAG, String.format("Running %1$s on %2$s", runCaseCommandStr, mRemoteDevice.getName()));
