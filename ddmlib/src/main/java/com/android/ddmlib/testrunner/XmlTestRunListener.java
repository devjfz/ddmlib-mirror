...
public class XmlTestRunListener implements ITestRunListener {
  ...
  private void generateDocument(File reportDir, long elapsedTime) {
    ...
    try {
      ...
      String msg =
          String.format(
              "XML test result file generated at %s. Total tests %d, " + "Failed %d, Error %d",
              getAbsoluteReportPath(),
              mRunResult.getNumTests(),
              mRunResult.getNumFailedTests(),
              mRunResult.getNumErrorTests());
