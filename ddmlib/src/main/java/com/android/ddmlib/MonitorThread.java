...
final class MonitorThread extends Thread {
  ...
  /**
   * Returns "true" if we want to retry connections to clients if we get a bad JDWP handshake back,
   * "false" if we want to just mark them as bad and leave them alone.
   */
  ...
  public void run() {
    ...
    try {
      ...
      Log.logAndDisplay(
          LogLevel.ERROR, "ddms", "Failed to initialize Monitor Thread: " + ioe.getMessage());
      return;
    }
    ...
    try {
      ...
      while (packet != null) {
        if (packet.isDdmPacket()) {
          ...
          if (packet.isError()) client.packetFailed(packet);
          else if (packet.isEmpty())
            Log.d(
                "ddms",
                "Got empty reply for 0x" + Integer.toHexString(packet.getId()) + " from " + client);
          else callHandler(client, packet, handler);
          packet.consume();
  ...
  private void callHandler(Client client, JdwpPacket packet, ChunkHandler handler) {
    ...
    if (!client.ddmSeen()) broadcast(CLIENT_READY, client);
    ...
    if (handler == null) {
      Log.w(
          "ddms",
          "Received unsupported chunk type " + ChunkHandler.name(type) + " (len=" + length + ")");
    } else {
      Log.d(
          "ddms",
          "Calling handler for "
  ...
  /**
   * Drops the provided list of clients from the monitor. This will lock the {@link Client} list of
   * the {@link Device} running each of the clients.
   */
    ...
    try {
      ...
      if (client.isDdmAware()) {
        ...
        client
            .getDeviceImpl()
            .getMonitor()
            .addClientToDropAndReopen(client, IDebugPortProvider.NO_STATIC_PORT);
      } else {
        Log.d("ddms", " (recycling client connection as well)");
        // we should drop the client, but also attempt to reopen it.
        // This is done by the DeviceMonitor.
