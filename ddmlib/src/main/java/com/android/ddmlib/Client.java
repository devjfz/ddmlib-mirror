...
public class Client {
  ...
  public void dumpHprof() {
    ...
    try {
      if (canStream) {
        ...
        String file =
            "/sdcard/" + mClientData.getClientDescription().replaceAll("\\:.*", "") + ".hprof";
   ...
   * IClientChangeListener#clientChanged(Client, int)} with a <code>changeMask</code> containing the
   * mask {@link #CHANGE_HEAP_ALLOCATION_STATUS}.
   */
   ...
   *
   * <p>If <code>true</code> the VM will be able to send thread information. Thread information must
   * be requested with {@link #requestThreadUpdate()}.
   *
   ...
   * IClientChangeListener#clientChanged(Client, int)} with a <code>changeMask</code> containing the
   * mask {@link #CHANGE_THREAD_STACKTRACE}.
   */
  ...
  public void setHeapUpdateEnabled(boolean enabled) {
    ...
    try {
      ...
      HandleHeap.sendHPSG(
          this, enabled ? HandleHeap.WHEN_GC : HandleHeap.WHEN_DISABLE, HandleHeap.WHAT_MERGE);
    } catch (IOException ioe) {
      // ignore it here; client will clean up shortly
    }
   ...
   * IClientChangeListener#clientChanged(Client, int)} with a <code>changeMask</code> containing the
   * mask {@link #CHANGE_NATIVE_HEAP_DATA}.
   */
  ...
  /**
   * Sends a request to the VM to send the information about all the allocations that have happened
   * since the call to {@link #enableAllocationTracker(boolean)} with <var>enable</var> set to
   * <code>null</code>. This is asynchronous.
   *
   * <p>The allocation information can be accessed by {@link ClientData#getAllocations()}. The
   * notification that the new data is available will be received through {@link
   * IClientChangeListener#clientChanged(Client, int)} with a <code>changeMask</code> containing the
   * mask {@link #CHANGE_HEAP_ALLOCATIONS}.
   */
   ...
   *
   * <p>Equivalent to sendAndConsume(packet, null).
   */
   ...
   * prevent anybody else from writing to the channel until this packet completes, so we synchronize
   * on the channel.
   *
    ...
    if (mConnState == ST_AWAIT_SHAKE) {
      ...
      switch (result) {
        case JdwpPacket.HANDSHAKE_GOOD:
          Log.d("ddms", "Good handshake from client, sending HELO to " + mClientData.getPid());
        ...
        case JdwpPacket.HANDSHAKE_BAD:
          ...
          if (MonitorThread.getInstance().getRetryOnBadHandshake()) {
            ...
            mDevice.getMonitor().addClientToDropAndReopen(this, IDebugPortProvider.NO_STATIC_PORT);
          } else {
  ...
  private void addRequestId(int id, ChunkHandler handler) {
    synchronized (mOutstandingReqs) {
      if (Log.Config.LOGV) Log.v("ddms", "Adding req 0x" + Integer.toHexString(id) + " to set");
  ...
  void removeRequestId(int id) {
    synchronized (mOutstandingReqs) {
      if (Log.Config.LOGV) Log.v("ddms", "Removing req 0x" + Integer.toHexString(id) + " from set");
  ...
  /**
   * Determine whether this is a response to a request we sent earlier. If so, return the
   * ChunkHandler responsible.
   */
  ...
  /**
   * An earlier request resulted in a failure. This is the expected response to a HELO message when
   * talking to a non-DDM client.
   */
   ...
   * broadcast to the handlers when a client connection is made. This method is synchronized so that
   * we only send the broadcast once.
   */
