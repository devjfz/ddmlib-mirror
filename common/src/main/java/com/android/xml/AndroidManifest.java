...
public final class AndroidManifest {
   ...
   * @return An IAbstractFile object pointing to the manifest or null if the manifest is missing.
   */
  ...
  public static String getPackage(IAbstractFile manifestFile)
    ...
    return xPath.evaluate(
        "/" + NODE_MANIFEST + "/@" + ATTRIBUTE_PACKAGE,
   ...
   *
   * <p>If the give manifest does not contain the debuggable attribute then the application is
   * considered to not be debuggable.
   *
   * @param manifestFile the manifest to parse.
  ...
  /**
   * Returns the value of the targetSdkVersion attribute (defaults to 1 if the attribute is not
   * set), or -1 if the value is a codename.
   *
   * @param manifestFile the manifest file to read the attribute from.
   * @return the integer value or -1 if not set.
   * @throws XPathExpressionException
  ...
  /**
   * Given a fully qualified activity name (e.g. com.foo.test.MyClass) and given a project package
   * base name (e.g. com.foo), returns the relative activity name that would be used the "name"
   * attribute of an "activity" element.
   *
